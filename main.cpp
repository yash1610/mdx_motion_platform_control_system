...
int main(int argc, char *argv[])
{
	...
	printf("kp = %f\nki = %f\nkd = %f\n", kp, ki, kd);
	double sample_rate = 10; //milliseconds
	double min_val = 0;		 //millibars
	double max_val = 6000;	 //millibars
	PID pid(sample_rate, kp, ki, kd, max_val, min_val);
	...
	while (i == 0)
	{
		...
		if (current_time - pid_loop_previous_time >= sample_rate)
		{
			...
			current_time = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();
			...
			if (current_time - previous_time >= setpoint_frequency)
			{
				...
				setpoints[0] = setpoints[3] + (std::rand() % 21) - 10;
				setpoints[1] = setpoints[3] + (std::rand() % 21) - 10;
				setpoints[2] = setpoints[3] + (std::rand() % 21) - 10;
				setpoints[4] = setpoints[3] + (std::rand() % 21) - 10;
				setpoints[5] = setpoints[3] + (std::rand() % 21) - 10;
			}
